...
| `notifications`             | ✅ built    | In-app notifications derived from the audit stream (a `NotificationAuditSink` is bound as every module's audit sink, so producers are unchanged). Multi-provider delivery (email/SMS/WhatsApp via Twilio/Gupshup/Heydoot behind a `MessageProvider` port; per-tenant channel config in org settings, ADR 0010). `/v1/notifications`, `/v1/notifications/channels`. |
