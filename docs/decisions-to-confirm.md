...
| 10  | Messaging credentials      | Platform-level provider accounts (current — per-tenant sender only) vs per-tenant provider credentials. The latter needs a secret-storage decision (KMS/vault) before tenant-supplied secrets are accepted (ADR 0010). | Notifications, Messaging      |
| 11  | Messaging consent/opt-out  | Per-channel marketing vs transactional consent and opt-out handling for customer-facing SMS/WhatsApp (DLT/WhatsApp template rules, esp. India). Routing rules (event→channel→recipient) gate on this.                  | Notifications, GDPR           |
